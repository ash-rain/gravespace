<x-layouts.app :title="__('Edit Memorial') . ' — ' . $memorial->full_name">
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
            ...
            <div class="flex items-center gap-3">
                @if($memorial->is_published)
                    <a href="{{ route('memorial.show', $memorial) }}" target="_blank" class="inline-flex items-center gap-2 px-4 py-2.5 bg-elevated border border-border text-text-muted text-sm font-medium rounded-xl hover:text-text transition-colors">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <form method="POST" action="{{ route('dashboard.memorials.update', $memorial) }}" enctype="multipart/form-data" class="space-y-8">
            @csrf
            @method('PUT')
            ...
            <div class="bg-surface border border-border rounded-xl p-6 sm:p-8">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="first_name" id="first_name" value="{{ old('first_name', $memorial->first_name) }}" required
                    ...
                    <div>
                        ...
                        <input type="text" name="last_name" id="last_name" value="{{ old('last_name', $memorial->last_name) }}" required
                               class="w-full px-4 py-3 bg-elevated border border-border rounded-xl text-text placeholder-text-muted/50 focus:outline-none focus:border-accent focus:ring-1 focus:ring-accent transition-colors">
                        @error('last_name')
                            <p class="mt-2 text-sm text-red-400">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <input type="text" name="maiden_name" id="maiden_name" value="{{ old('maiden_name', $memorial->maiden_name) }}"
                               class="w-full px-4 py-3 bg-elevated border border-border rounded-xl text-text placeholder-text-muted/50 focus:outline-none focus:border-accent focus:ring-1 focus:ring-accent transition-colors">
                        @error('maiden_name')
                            <p class="mt-2 text-sm text-red-400">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <input type="date" name="date_of_birth" id="date_of_birth" value="{{ old('date_of_birth', $memorial->date_of_birth?->format('Y-m-d')) }}" required
                    ...
                    <div>
                        ...
                        <input type="date" name="date_of_death" id="date_of_death" value="{{ old('date_of_death', $memorial->date_of_death?->format('Y-m-d')) }}" required
                               class="w-full px-4 py-3 bg-elevated border border-border rounded-xl text-text focus:outline-none focus:border-accent focus:ring-1 focus:ring-accent transition-colors [color-scheme:dark]">
                        @error('date_of_death')
                            <p class="mt-2 text-sm text-red-400">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <input type="text" name="place_of_birth" id="place_of_birth" value="{{ old('place_of_birth', $memorial->place_of_birth) }}"
                               class="w-full px-4 py-3 bg-elevated border border-border rounded-xl text-text placeholder-text-muted/50 focus:outline-none focus:border-accent focus:ring-1 focus:ring-accent transition-colors"
                    ...
                    <div>
                        ...
                        <input type="text" name="place_of_death" id="place_of_death" value="{{ old('place_of_death', $memorial->place_of_death) }}"
                               class="w-full px-4 py-3 bg-elevated border border-border rounded-xl text-text placeholder-text-muted/50 focus:outline-none focus:border-accent focus:ring-1 focus:ring-accent transition-colors"
                               placeholder="{{ __('City, State, Country') }}">
                    ...
                    <div>
                        ...
                        @if($memorial->cover_photo)
                            <div class="mb-3 relative rounded-xl overflow-hidden">
                                <img src="{{ Storage::url($memorial->cover_photo) }}" alt="{{ __('Current cover photo') }}" class="w-full h-32 object-cover rounded-xl">
                                <span class="absolute bottom-2 left-2 text-xs bg-primary/80 text-text-muted px-2 py-1 rounded-lg">{{ __('Current') }}</span>
                            </div>
                        @endif
                        ...
                        <p class="mt-2 text-xs text-text-muted">{{ __('Upload a new image to replace the current one.') }}</p>
                        @error('cover_photo')
                            <p class="mt-2 text-sm text-red-400">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        @if($memorial->profile_photo)
                            <div class="mb-3 relative">
                                <img src="{{ Storage::url($memorial->profile_photo) }}" alt="{{ __('Current profile photo') }}" class="w-24 h-24 object-cover rounded-xl">
                                <span class="absolute bottom-1 left-1 text-xs bg-primary/80 text-text-muted px-2 py-0.5 rounded-lg">{{ __('Current') }}</span>
                            </div>
                        @endif
            ...
            <div class="bg-surface border border-border rounded-xl p-6 sm:p-8">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    <div class="sm:col-span-2">
                        ...
                        <input type="text" name="cemetery_name" id="cemetery_name" value="{{ old('cemetery_name', $memorial->cemetery_name) }}"
                               class="w-full px-4 py-3 bg-elevated border border-border rounded-xl text-text placeholder-text-muted/50 focus:outline-none focus:border-accent focus:ring-1 focus:ring-accent transition-colors"
                    ...
                    <div class="sm:col-span-2">
                        ...
                        <input type="text" name="cemetery_address" id="cemetery_address" value="{{ old('cemetery_address', $memorial->cemetery_address) }}"
                               class="w-full px-4 py-3 bg-elevated border border-border rounded-xl text-text placeholder-text-muted/50 focus:outline-none focus:border-accent focus:ring-1 focus:ring-accent transition-colors"
                    ...
                    <div>
                        ...
                        <input type="number" step="any" name="latitude" id="latitude" value="{{ old('latitude', $memorial->latitude) }}"
                               class="w-full px-4 py-3 bg-elevated border border-border rounded-xl text-text placeholder-text-muted/50 focus:outline-none focus:border-accent focus:ring-1 focus:ring-accent transition-colors"
                    ...
                    <div>
                        ...
                        <input type="number" step="any" name="longitude" id="longitude" value="{{ old('longitude', $memorial->longitude) }}"
                               class="w-full px-4 py-3 bg-elevated border border-border rounded-xl text-text placeholder-text-muted/50 focus:outline-none focus:border-accent focus:ring-1 focus:ring-accent transition-colors"
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        ...
                        <div class="flex items-center gap-2">
                            ...
                            <input type="text" name="slug" id="slug" value="{{ old('slug', $memorial->slug) }}"
                        ...
                        @enderror
                        @if(!Auth::user()->isPremium())
                            <p class="mt-2 text-xs text-text-muted">{{ __('Upgrade to Premium to customize your memorial URL.') }}</p>
                        @endif
                    </div>
                    ...
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <p class="text-text-muted text-xs mt-1">{{ __('Generate a QR code that links directly to this memorial.') }}</p>
                        </div>
                        @if(Auth::user()->isPremium())
                            <a href="{{ route('dashboard.memorials.qr', $memorial) }}" class="inline-flex items-center gap-2 px-4 py-2.5 bg-elevated border border-border text-text-muted text-sm font-medium rounded-xl hover:text-accent hover:border-accent/30 transition-colors">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        @else
                            <a href="{{ route('dashboard.billing') }}" class="inline-flex items-center gap-2 px-4 py-2.5 bg-elevated border border-border text-text-muted text-sm font-medium rounded-xl opacity-60 hover:opacity-100 transition-opacity">
                                {{ __('Upgrade') }}
                            </a>
                        @endif
                    </div>
        ...
        <div class="mt-12 bg-surface border border-border rounded-xl p-6 sm:p-8">
                ...
                <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4 mb-6">
                    @foreach($memorial->photos as $photo)
                        <div class="relative group rounded-xl overflow-hidden aspect-square bg-elevated">
                            <img src="{{ Storage::url($photo->path) }}" alt="{{ $photo->caption ?? __('Memorial photo') }}" class="w-full h-full object-cover">
                            <div class="absolute inset-0 bg-primary/60 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                                <form method="POST" action="{{ route('dashboard.photos.destroy', $photo) }}" onsubmit="return confirm('{{ __('Delete this photo?') }}')">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="p-2 bg-red-500/20 border border-red-500/30 text-red-400 rounded-xl hover:bg-red-500/30 transition-colors">
            ...
            {{-- Upload More Photos --}}
            <form method="POST" action="{{ route('dashboard.memorials.photos.store', $memorial) }}" enctype="multipart/form-data" class="border-t border-border pt-6">
                @csrf
                <label for="photos" class="block text-sm font-medium text-text-muted mb-2">{{ __('Add Photos to Gallery') }}</label>
                <div class="flex flex-col sm:flex-row items-start sm:items-end gap-4">
                    <div class="flex-1 w-full">
                        <input type="file" name="photos[]" id="photos" accept="image/*" multiple
                               class="w-full px-4 py-3 bg-elevated border border-border rounded-xl text-text file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-medium file:bg-accent/10 file:text-accent hover:file:bg-accent/20 focus:outline-none focus:border-accent transition-colors">
                        <p class="mt-2 text-xs text-text-muted">{{ __('Select multiple images. JPG, PNG, or WebP.') }}</p>
                    </div>
                    <button type="submit" class="px-6 py-3 bg-accent text-primary font-semibold text-sm rounded-xl hover:bg-accent/90 transition-colors shrink-0">
                        {{ __('Upload Photos') }}
                    </button>
                </div>
            </form>
                            ...
                            <div class="flex items-start justify-between gap-4">
                                <div class="flex-1 min-w-0">
                                    <div class="flex items-center gap-3 mb-2">
                                        <span class="text-text font-medium text-sm">{{ $tribute->author_name }}</span>
                                ...
                                <div class="flex items-center gap-2 shrink-0">
                                    @if(!$tribute->is_approved)
                                        <form method="POST" action="{{ route('dashboard.tributes.approve', $tribute) }}">
                                            @csrf
                                            <button type="submit" class="p-2 bg-green-500/10 border border-green-500/30 text-green-400 rounded-xl hover:bg-green-500/20 transition-colors" title="{{ __('Approve') }}">
                                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        ...
                                        <form method="POST" action="{{ route('dashboard.tributes.reject', $tribute) }}">
                                            @csrf
                                            <button type="submit" class="p-2 bg-yellow-500/10 border border-yellow-500/30 text-yellow-400 rounded-xl hover:bg-yellow-500/20 transition-colors" title="{{ __('Reject') }}">
                                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    ...
                                    @endif
                                    <form method="POST" action="{{ route('dashboard.tributes.destroy', $tribute) }}" onsubmit="return confirm('{{ __('Delete this tribute?') }}')">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="p-2 bg-red-500/10 border border-red-500/30 text-red-400 rounded-xl hover:bg-red-500/20 transition-colors" title="{{ __('Delete') }}">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/>
                                            </svg>
                                        </button>
        ...
        <div class="mt-8 bg-red-500/5 border border-red-500/20 rounded-xl p-6 sm:p-8">
            ...
            <p class="text-text-muted text-sm mb-6">{{ __('Once you delete a memorial, there is no going back. All photos, tributes, and data will be permanently removed.') }}</p>

            <form method="POST" action="{{ route('dashboard.memorials.destroy', $memorial) }}" onsubmit="return confirm('{{ __('Are you absolutely sure you want to delete this memorial? This action is irreversible.') }}')" x-data>
                @csrf
                @method('DELETE')
                <button type="submit" class="inline-flex items-center gap-2 px-6 py-3 bg-red-500/10 border border-red-500/30 text-red-400 font-semibold text-sm rounded-xl hover:bg-red-500/20 transition-colors">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/>
                    </svg>
