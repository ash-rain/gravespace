<x-layouts.app :title="__('Dashboard')">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        ...
        <div class="mb-8">
            ...
            @else
                <div
                    class="bg-surface border border-border rounded-xl p-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                    <div>
                        <h3 class="font-serif text-lg font-semibold text-text">{{ __('Upgrade to Premium') }}</h3>
                        <p class="text-text-muted text-sm mt-1">
                            {{ __('Unlock unlimited memorials, custom QR codes, custom slugs, and more.') }}</p>
                    </div>
                    <a href="{{ route('dashboard.billing') }}"
                        class="inline-flex items-center justify-center px-6 py-3 bg-accent text-primary font-semibold text-sm rounded-xl hover:bg-accent/90 transition-colors shrink-0">
                        {{ __('View Plans') }}
                    </a>
                </div>
            @endif
            ...
            <div class="bg-surface border border-border rounded-xl p-6">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="mt-2 text-3xl font-bold font-serif text-text">{{ $memorials->sum('photos_count') }}
                        </p>
                    </div>
                    <div class="w-12 h-12 bg-elevated rounded-xl flex items-center justify-center">
                        <svg class="w-6 h-6 text-accent" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="mb-10">
            ...
            <div class="flex flex-wrap gap-4">
                ...
                <a href="{{ route('dashboard.billing') }}"
                    class="inline-flex items-center gap-2 px-6 py-3 bg-elevated border border-border text-text font-semibold text-sm rounded-xl hover:bg-surface transition-colors">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div>
            ...
            @if ($memorials->isEmpty())
                <div class="bg-surface border border-border rounded-xl p-10 text-center">
                    <svg class="w-12 h-12 text-text-muted mx-auto mb-4" fill="none" stroke="currentColor"
                        viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"
                            d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253" />
                    </svg>
                    ...
                    @foreach ($memorials->take(5) as $memorial)
                        <div class="flex items-center justify-between p-5">
                                ...
                                <div class="w-12 h-12 bg-elevated rounded-xl overflow-hidden shrink-0">
                                    @if ($memorial->profile_photo)
                                        <img src="{{ Storage::url($memorial->profile_photo) }}"
                                            alt="{{ $memorial->fullName() }}" class="w-full h-full object-cover">
                                    @else
                            ...
                            <div class="flex items-center gap-3">
                                <span
                                    class="inline-flex items-center px-2.5 py-1 rounded-lg text-xs font-medium {{ $memorial->is_published ? 'bg-green-500/10 text-green-400' : 'bg-yellow-500/10 text-yellow-400' }}">
