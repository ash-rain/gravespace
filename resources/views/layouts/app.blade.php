                    ...
                    <div class="flex justify-between h-16">
                        <div class="flex items-center">
                            ...
                            <div class="hidden sm:flex sm:ml-10 sm:gap-8">
                                <a href="{{ route('dashboard.index') }}" class="inline-flex items-center px-1 pt-1 text-sm font-medium {{ request()->routeIs('dashboard.index') ? 'text-accent border-b-2 border-accent' : 'text-text-muted hover:text-text' }}">
                                    {{ __('Dashboard') }}
                                </a>
                            ...
                            <div class="relative" x-data="{ open: false }">
                                ...
                                <div x-show="open" @click.away="open = false" x-transition class="absolute right-0 mt-2 w-48 bg-elevated border border-border rounded-lg shadow-xl z-50">
                                    ...
                                    <form method="POST" action="{{ route('logout') }}">
                                        @csrf
                                        <button type="submit" class="w-full text-left px-4 py-2 text-sm text-text-muted hover:text-text hover:bg-surface">{{ __('Log Out') }}</button>
                                    </form>
                                </div>
                            </div>
                        </div>
