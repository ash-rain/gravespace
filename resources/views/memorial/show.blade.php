<x-layouts.memorial>
                ...
                <div class="flex flex-col sm:flex-row items-center sm:items-end gap-4 sm:gap-6">
                    ...
                    <div class="shrink-0">
                        @if($memorial->profile_photo)
                            ...
                            >
                        @else
                            <div class="w-32 h-32 sm:w-36 sm:h-36 rounded-full border-4 border-primary bg-elevated flex items-center justify-center shadow-2xl">
            ...
            <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
                <div class="flex flex-wrap items-center justify-center sm:justify-start gap-6">
                    ...
                    <div class="flex items-center gap-2 text-text-muted">
                        ...
                        <span class="text-sm font-medium">{{ $giftCounts->get('candle', 0) }} {{ __('candles') }}</span>
                    </div>
                    <div class="flex items-center gap-2 text-text-muted">
                        <span class="text-xl" title="{{ __('Flowers') }}">&#x1F339;</span>
                        <span class="text-sm font-medium">{{ $giftCounts->get('flower', 0) }} {{ __('flowers') }}</span>
                    </div>
                    <div class="flex items-center gap-2 text-text-muted">
                ...
                <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
                    <div class="flex items-center justify-between mb-6">
                        ...
                        @if($memorial->photos->count() > 6)
                            <a href="{{ route('memorial.gallery', $memorial->slug) }}" class="text-sm text-accent hover:text-accent-hover transition-colors">
                    ...
                    <div class="grid grid-cols-2 sm:grid-cols-3 gap-3">
                        @foreach($memorial->photos->take(6) as $photo)
                            <div class="relative aspect-square bg-elevated rounded-xl overflow-hidden group">
                                ...
                                >
                                @if($photo->caption)
                                    <div class="absolute inset-x-0 bottom-0 bg-gradient-to-t from-primary/80 to-transparent p-3 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                ...
                <div class="bg-surface border border-border rounded-2xl p-6 sm:p-8">
                    <form method="POST" action="{{ route('memorial.gifts.store', $memorial->slug) }}">
                        ...
                        <div class="flex flex-wrap gap-3 mb-6">
                            <button type="button" @click="selectedGift = 'candle'" class="flex items-center gap-2 px-4 py-2.5 rounded-xl border text-sm transition-all duration-200" :class="selectedGift === 'candle' ? 'bg-accent/10 border-accent/50 text-accent' : 'bg-elevated border-border text-text-muted hover:border-accent/30'">
                                <span class="text-lg">&#x1F56F;</span> {{ __('Candle') }}
                            </button>
                            <button type="button" @click="selectedGift = 'flower'" class="flex items-center gap-2 px-4 py-2.5 rounded-xl border text-sm transition-all duration-200" :class="selectedGift === 'flower' ? 'bg-accent/10 border-accent/50 text-accent' : 'bg-elevated border-border text-text-muted hover:border-accent/30'">
                                <span class="text-lg">&#x1F339;</span> {{ __('Flower') }}
                            </button>
                            <button type="button" @click="selectedGift = 'tree'" class="flex items-center gap-2 px-4 py-2.5 rounded-xl border text-sm transition-all duration-200" :class="selectedGift === 'tree' ? 'bg-accent/10 border-accent/50 text-accent' : 'bg-elevated border-border text-text-muted hover:border-accent/30'">
                                <span class="text-lg">&#x1F333;</span> {{ __('Tree') }}
                            </button>
                            <button type="button" @click="selectedGift = 'wreath'" class="flex items-center gap-2 px-4 py-2.5 rounded-xl border text-sm transition-all duration-200" :class="selectedGift === 'wreath' ? 'bg-accent/10 border-accent/50 text-accent' : 'bg-elevated border-border text-text-muted hover:border-accent/30'">
                                <span class="text-lg">&#x1FAB7;</span> {{ __('Wreath') }}
                            </button>
                            <button type="button" @click="selectedGift = 'star'" class="flex items-center gap-2 px-4 py-2.5 rounded-xl border text-sm transition-all duration-200" :class="selectedGift === 'star' ? 'bg-accent/10 border-accent/50 text-accent' : 'bg-elevated border-border text-text-muted hover:border-accent/30'">
                        ...
                        {{-- Optional Message --}}
                        <div class="mb-6">
                            <label for="gift_message" class="block text-sm font-medium text-text-muted mb-2">{{ __('Message (optional)') }}</label>
                            <textarea
                                id="gift_message"
                                name="message"
                                rows="2"
                        ...
                        <button type="submit" class="inline-flex items-center gap-2 px-6 py-2.5 text-sm font-semibold bg-accent hover:bg-accent-hover text-primary rounded-lg transition-colors duration-200">
                            {{ __('Leave Gift') }}
                        </button>
                    </form>
                    ...
                    <form method="POST" action="{{ route('memorial.tributes.store', $memorial->slug) }}">
                        ...
                        @guest
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
                                <div>
                                    <label for="author_name" class="block text-sm font-medium text-text-muted mb-1.5">{{ __('Your Name') }}</label>
                                    <input
                                        type="text"
                                        id="author_name"
                                        name="author_name"
                                ...
                                <div>
                                    <label for="author_email" class="block text-sm font-medium text-text-muted mb-1.5">{{ __('Email (private)') }}</label>
                                    <input
                                        type="email"
                                        id="author_email"
                                        name="author_email"
                        ...
                        <div class="mb-4">
                            <label for="tribute_body" class="block text-sm font-medium text-text-muted mb-1.5">{{ __('Your Tribute') }}</label>
                            <textarea
                                id="tribute_body"
                                name="body"
                                rows="4"
                                required
                                class="w-full bg-elevated border border-border rounded-xl px-4 py-3 text-text placeholder:text-text-muted/50 focus:outline-none focus:border-accent/50 focus:ring-1 focus:ring-accent/50 text-sm resize-none transition-colors"
                                placeholder="{{ __('Share a memory, a story, or a message...') }}"
                            >{{ old('body') }}</textarea>
                        </div>
                        ...
                        <div class="flex items-center justify-between">
                            ...
                            <p class="text-xs text-text-muted">{{ __('Tributes are reviewed before publishing.') }}</p>
                        </div>
                    </form>
                </div>
                        ...
                        @foreach($memorial->approvedTributes as $tribute)
                            <div class="bg-surface border border-border rounded-xl p-6">
                                <div class="flex items-start justify-between mb-3">
                                    <div>
                                        <p class="text-sm font-medium text-text">{{ $tribute->authorDisplayName() }}</p>
                                        <p class="text-xs text-text-muted">{{ $tribute->created_at->diffForHumans() }}</p>
                                    </div>
                                </div>
    ...
    {{-- Success Flash --}}
    @if(session('success'))
        <div class="fixed bottom-6 right-6 z-50 bg-elevated border border-accent/50 text-text px-6 py-4 rounded-xl shadow-2xl max-w-sm" x-data="{ show: true }" x-show="show" x-init="setTimeout(() => show = false, 5000)" x-transition>
